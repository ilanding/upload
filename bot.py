...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📌 *Kaise use karein:*\n"
        f"• Koi bhi `.apk` file bhejo (kisi bhi naam ki)\n"
        f"• Main usse `{FIXED_FILE_NAME}` naam se GitHub pe save karunga\n"
        f"• Purani file replace ho jaayegi, link hamesha same rahega\n\n"
        f"📂 *Repo:* `{GITHUB_REPO}`\n"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Step 1: Received (show original name, but will be renamed)
    ...
    # Step 3: Uploading to GitHub (using fixed name, not original)
    await status_msg.edit_text(
    ...
    # Step 4: Result
    if success:
        download_url = f"https://github.com/{GITHUB_REPO}/raw/{GITHUB_BRANCH}/{FIXED_FILE_NAME}"
