...
def update_install_url(html: str, new_url: str):
    ...
    button_pattern = r'(<a\b[^>]*\bclass\s*=\s*["\'][^"\']*\bbtn\b[^"\']*["\'][^>]*\bhref\s*=\s*["\'])[^"\']*(["\'])'
    ...
    redirect_pattern = r'(window\.location\.replace\s*\(\s*["\'])[^"\']*(["\']\s*\))'
    html, redirect_count = re.subn(
        redirect_pattern,
        lambda m: m.group(1) + new_url + m.group(2),
        html, count=1, flags=re.IGNORECASE | re.DOTALL,
    )

    return html, (button_count > 0 and redirect_count > 0)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• It will replace the Download button URL and automatic redirect URL\n"
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "4️⃣ Download button aur automatic redirect — dono URL replace karega\n"
...
async def handle_url_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not changed:
            await status_msg.edit_text(
                "❌ *Download/Redirect URL was not found!*\n\n"
                "Expected both a `<a class=\"btn\" href=...>` and a `window.location.replace(...)` URL.\n\n"
        ...
        await status_msg.edit_text(
            "✏️ *Download + Redirect URL have been replaced!*\n\n"
